...
bool Gui::AddWindow(const String &sName, Awesomium::WebSession *pSession, const bool &pVisible, const String &sUrl, const int &nWidth, const int &nHeight, const int &nX, const int &nY, const String &sGlobalJSObjectName, const bool &bTransparent, const bool &bEnabled)
{
	...
	{
		// awesomium is not initialized, therefor we should not do anything
		return false;
	}
}
...
void Gui::Initialize()
{
	// we create a awesomium web config
	Awesomium::WebConfig cWebConfig;
	// we try to initialize awesomium
	m_pAwesomiumWebCore = Awesomium::WebCore::Initialize(cWebConfig);
	if (m_pAwesomiumWebCore)
	{
		m_bAwesomiumInitialized = true;
	}
	...
	{
		// awesomium could not be initialized
	}
}


void Gui::StopAwesomium() const
{
	if (m_bAwesomiumInitialized)
	{
		// stop awesomium
		m_pAwesomiumWebCore->Shutdown();
	}
}


Awesomium::WebView *Gui::GetAwesomiumWindow(const String &sName) const
{
	if (m_pWindows->Get(sName) == NULL)
	{
		// we should return the awesomium window from the dummy window to prevent crashes on called functions
		// the end user should always verify that the returned object is the right one
		return m_pWindows->Get(DUMMYWINDOW)->GetAwesomiumWindow();
	}
	else
	{
		// return the awesomium window
		return m_pWindows->Get(sName)->GetAwesomiumWindow();
	}
}


void Gui::UpdateAwesomium()
{
	// update awesomium
	m_pAwesomiumWebCore->Update();
}
...
bool Gui::IsAwesomiumInitialized() const
{
	return m_bAwesomiumInitialized;
}


void Gui::MouseMove(const SRPWindows *pSRPWindow, const Vector2i &vMousePos) const
{
	// move the mouse for awesomium
	pSRPWindow->GetAwesomiumWindow()->InjectMouseMove(pSRPWindow->GetRelativeMousePosition(vMousePos).x, pSRPWindow->GetRelativeMousePosition(vMousePos).y);
}
...
void Gui::FocusWindow(SRPWindows *pSRPWindows)
{
	...
	if (m_pFocusedWindow != pSRPWindows)
	{
		...
		// focus the window
		pSRPWindows->GetAwesomiumWindow()->Focus();
